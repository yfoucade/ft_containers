...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/07 11:56:50 by yfoucade          #+#    #+#             */
/*   Updated: 2023/02/07 14:46:13 by yfoucade         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
> class set
{
	// member types
	private:
		typedef RedBlackTree<Key, Key, Compare> data_struct;
		typedef typename RedBlackTree<Key, Key, Compare>::node_type node_type;
	...
	// private members
	private:
		// now _rbt won't change during lifetime of set instance.
	...
	public:
		...
		set( void );
		explicit set( const Compare&, const Allocator& = Allocator() );
...
template< typename Key, typename Compare, typename Allocator >
void
set<Key, Compare, Allocator>::destroy_tree()
{
	node_type* tmp;
	if (!_rbt)
		return;
...
template< typename Key, typename Compare, typename Allocator >
Allocator
set<Key, Compare, Allocator>::get_allocator( void ) const
{
	return _alloc;
}
...
set<Key, Compare, Allocator>::begin( void )
{
...
set<Key, Compare, Allocator>::begin( void ) const
{
...
template< typename Key, typename Compare, typename Allocator >
typename set<Key, Compare, Allocator>::iterator
set<Key, Compare, Allocator>::end( void )
{
...
template< typename Key, typename Compare, typename Allocator >
typename set<Key, Compare, Allocator>::const_iterator
set<Key, Compare, Allocator>::end( void ) const
{
...
set<Key, Compare, Allocator>::rbegin( void )
{
...
set<Key, Compare, Allocator>::rbegin( void ) const
{
...
template< typename Key, typename Compare, typename Allocator >
typename set<Key, Compare, Allocator>::reverse_iterator
set<Key, Compare, Allocator>::rend( void )
{
...
template< typename Key, typename Compare, typename Allocator >
typename set<Key, Compare, Allocator>::const_reverse_iterator
set<Key, Compare, Allocator>::rend( void ) const
{
	return ft::reverse_iterator<const_iterator>(begin());
}

template< typename Key, typename Compare, typename Allocator >
bool set<Key, Compare, Allocator>::empty( void ) const
{
	return _size == 0;
}
...
set<Key, Compare, Allocator>::insert( const value_type& value )
{
	...
	node_type* node = new node_type(RED, value, new_value, NULL, NULL, NULL);
	_rbt->insert(node);
	++_size;
...
template< typename Key, typename Compare, typename Allocator >
typename set<Key, Compare, Allocator>::size_type
set<Key, Compare, Allocator>::erase_node( node_type* node )
{
	if (!node)
		return 0;
	_rbt->remove(node);
...
template< typename Key, typename Compare, typename Allocator >
ft::pair< typename set<Key, Compare, Allocator>::iterator, typename set<Key, Compare, Allocator>::iterator>
set<Key, Compare, Allocator>::equal_range( const Key& key )
{
	return ft::pair<iterator, iterator>(lower_bound(key), upper_bound(key));
}

template< typename Key, typename Compare, typename Allocator >
ft::pair< typename set<Key, Compare, Allocator>::const_iterator, typename set<Key, Compare, Allocator>::const_iterator >
set<Key, Compare, Allocator>::equal_range( const Key& key ) const
{
...
set<Key, Compare, Allocator>::lower_bound( const Key& key )
{
	...
	while ( tmp != _rbt->get_nil() )
	{
		if ( _comp(tmp->key, key) )
			tmp = tmp->right;
		else if ( _comp(key, tmp->key) )
		{
			if ( ret == end() || _comp(tmp->key, *ret) )
...
template< typename Key, typename Compare, typename Allocator >
typename set<Key, Compare, Allocator>::size_type
set<Key, Compare, Allocator>::compute_ras( void )
{
...
{
	typename set<Key,Compare,Alloc>::const_iterator l1 = lhs.begin();
	typename set<Key,Compare,Alloc>::const_iterator l2 = rhs.begin();
	typename set<Key,Compare,Alloc>::const_iterator last = lhs.end();
                ...
                const ft::set<Key,Compare,Alloc>& rhs )
{
	...
	typename set<Key,Compare,Alloc>::const_iterator last1 = lhs.end();
	typename set<Key,Compare,Alloc>::const_iterator last2 = rhs.end();
